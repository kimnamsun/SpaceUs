		...
		<div
			...
			<div class="col-lg-8 col-md-6 ftco-animate d-flex align-items-end">
				<div class="text text-center mx-auto" style="margin-bottom: 80%;">
					...
					<p class="h6">
						소모임 게시판은 사람과 사람을 연결하고 space와 us를 연결하기 위한 자유 게시판으로,<br>
						SpaceUs에서는 정보교환의 온라인 공간을 제공할 뿐 중개에 관여하지 않으며,<br> 그에 따른 과실 또는
						손해발생에 대해 일체 책임을 지지 않음을 알려드립니다.!!!!!!!!
					</p>
				</div>
			</div>
		</div>
	...
	<div class="row m-5">
		<div class="col-12">
			<div class="m-5">
				<aside id="sideinfo" class="w-25 pr-3" style="margin-top: 60px;">
					<div class="list-group">
						<div class="list-group-item list-group-item-action"
							onclick="location.href='${pageContext.request.contextPath }/community/group/groupList.do'">
							전체보기 <span class="badge badge-primary badge-pill">${totalCnt}</span>
						</div>
						<c:forEach items="${boardList}" var="board" varStatus="vs">
							<form
								onclick="location.href='${pageContext.request.contextPath }/community/group/groupList/${board.boardNo}/${board.boardRef}.do'">
								<c:if test="${board.boardLevel == 1}">
									<div class="list-group-item list-group-item-action upper-list"
										id="upper">
										${board.boardName} <i class="fas fa-caret-down"
											style="padding: 0; position: relative; right: 0;"></i>
									</div>
								</c:if>
								<c:if test="${board.boardLevel == 2}">
									<div class="list-group-item list-group-item-action sub-list">
										<i class="fas fa-arrow-right"></i> ${board.boardName}
									</div>
								</c:if>
				...
				<div>
					<a
						href="${pageContext.request.contextPath }/community/group/groupEnrollForm.do"
							...
							<tbody>
								<c:forEach items="${groupBoardList}" var="gb" varStatus="vs">
									<tr style="cursor: pointer;"
										onclick="location.href='${pageContext.request.contextPath }/community/group/groupDetail/${gb.groupBoardNo}.do'">
										<td class="text-center">${vs.count}</td>
										<td class="txt-oflo"><span class="text-success mr-2">
												[${ gb.boardNo=="1"?
			                            			"전체 크루원":gb.boardNo=="2"?
			                            			"카페 크루원":gb.boardNo=="3"?
			                            			"무비 크루원":gb.boardNo=="4"?
			                            			...
			                            			"소모임 자랑":"기타"}]
										</span> ${gb.groupBoardTitle} &nbsp;&nbsp;[${gb.commentCnt}]</td>
